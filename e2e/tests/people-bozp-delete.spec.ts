...
test.describe("People – BOZP delete guard", () => {
  ...
  test("shows BOZP blocking message in toast when deleting a worker with PPE records", async ({
    ...
    try {
      ...
      const assignRes = await page.request.post("/api/ppe/assignments", {
        data: {
          ppeItemId: item.id,
          personId: person.id,
          quantity: 1,
      ...
      if (assignmentId !== undefined) await cleanupPpeAssignment(page.request, assignmentId);
      if (itemId !== undefined) await cleanupPpeItem(page.request, itemId);
      if (personId !== undefined) await cleanupPerson(page.request, personId);
    }
  });
});
