...
- `billing_documents` was missing the partial unique index `billing_documents_sha256_unique_idx` (`ON billing_documents(sha256) WHERE sha256 IS NOT NULL`, from `lib/db/migrations/0067_rich_talkback.sql`). Its absence let concurrent-dedup tests silently insert real duplicate rows instead of racing into `23505`. If creating it fails with a duplicate-key error, first delete the debris rows it's blocking on (leftover from prior test runs that ran without the constraint) before retrying. **This index has now recurred missing at least twice (June and July 2026)** — it is NOT durably fixed by re-creating it once; something in this dev DB's lifecycle drops it. Always re-check `pg_indexes` for it before trusting sha256-dedup test results.
...
- `email_import_log.attempts` (integer) is missing as of July 2026 — breaks `email-import-retry.test.ts`. NOT fixed yet (out of scope of the session that found it); add via `ALTER TABLE email_import_log ADD COLUMN IF NOT EXISTS attempts integer NOT NULL DEFAULT 0` mirroring the schema file before trusting that test.
- Two pre-existing, DB-independent test failures were also found unrelated to schema drift: `test/warehouse-ledger.test.ts`'s `insertMaterial` helper never sets `warehouseItemId` (stale relative to the "stable warehouse_item_id FK" migration — `resolveItemForMaterial` now requires it and no longer falls back to name matching), and `test/ppe-contract.test.ts`'s confirmed-assignment guard (PATCH/DELETE after `employeeConfirmedAt` is set) returns 200/204 instead of 409. Both reproduce in isolation; neither is caused by DB drift.
