...
export const GetAdminHealthResponse = zod.object({
  ...
  "migrationParity": zod.boolean().describe('True when all exact known migration identities are applied and, in production, the runtime release-evidence lineage matches the live known\/opaque inventory'),
  "migrationControlParity": zod.boolean().nullable().describe('Exact production steady-release lineage match; null outside production'),
  "productionRuntimeBinding": zod.union([zod.object({
...
}),zod.null()]).describe('Secret-free exact production target, config, image, database and schema binding; null outside the production runtime.'),
  "expectedMigrations": zod.number(),
  "knownAppliedMigrations": zod.number().describe('Live rows matching an expected journal timestamp and SQL hash exactly'),
  "knownMigrationRowsSha256": zod.string().describe('SHA-256 of the canonical sorted exact-known migration identity array, or sha256:unknown when inventory cannot be read'),
  "opaqueAppliedMigrations": zod.number().describe('Live rows not consumed by the exact known migration set, including duplicates and hash drift'),
  "opaqueMigrationRowsSha256": zod.string().describe('SHA-256 of the canonical sorted opaque migration identity array, or sha256:unknown when inventory cannot be read'),
  "appliedMigrations": zod.number().describe('Total live journal rows; equals knownAppliedMigrations plus opaqueAppliedMigrations'),
...
export const ListJobsResponseItem = zod.object({
  ...
  "billingIntent": zod.union([zod.literal('billable'),zod.literal('not_billable'),zod.literal(null)]).nullable().describe('Customer-billing intent; null when the caller may not view billing data'),
  "billingExclusionReason": zod.string().nullable().describe('Required internal reason when billingIntent is not_billable'),
  "billingIntentChangedAt": zod.string().nullable().describe('ISO timestamp of the last billing-intent change'),
  "billingIntentChangedByUserId": zod.number().nullable().describe('User who last changed the billing intent'),
...
export const UpdateJobBillingIntentBody = zod.object({
  ...
  "reason": zod.string().min(updateJobBillingIntentBodyReasonMin).max(updateJobBillingIntentBodyReasonMax).nullish().describe('Required when excluding a job from customer billing')
})
...
export const ListInvoicesResponseItem = zod.object({
  ...
  "title": zod.string(),
  "date": zod.string(),
  "billingState": zod.enum(['draft', 'partial', 'ready', 'billed', 'linked']).optional()
})).optional()
})
...
export const CreateInvoiceBody = zod.object({
  "documentType": zod.enum(['standard', 'advance']).default(createInvoiceBodyDocumentTypeDefault),
  "customerId": zod.number(),
  "customerName": zod.string().nullish(),
  "customerIc": zod.string().nullish(),
  "customerDic": zod.string().nullish(),
  ...
  "sourceGrouping": zod.enum(['by_job', 'combined']).default(createInvoiceBodySourceGroupingDefault).describe('Keep source rows grouped per job or combine compatible commercial rows across jobs'),
  ...
  "lines": zod.array(zod.object({
  "id": zod.number().nullish().describe('Existing draft line id retained during in-place edits'),
  ...
  "iban": zod.string().nullish(),
  "bic": zod.string().nullish(),
  "currency": zod.string().min(createInvoiceBodyCurrencyMin).max(createInvoiceBodyCurrencyMax).nullish(),
  "variableSymbol": zod.string().nullish(),
  "constantSymbol": zod.string().nullish(),
  "specificSymbol": zod.string().nullish(),
  "vatModeDefault": zod.enum(['standard', 'reverse_charge', 'zero', 'non_vat']).optional(),
...
export const GetInvoiceResponse = zod.object({
  ...
  "sourceAllocations": zod.array(zod.object({
  "id": zod.number(),
  ...
  "status": zod.enum(['reserved', 'billed', 'included_in_lump_sum', 'not_charged', 'deferred', 'released', 'reversed']),
  "settlementMethod": zod.enum(['direct', 'included_in_lump_sum', 'not_charged', 'deferred']),
...
export const UpdateInvoiceBody = zod.object({
  "customerId": zod.number().nullish(),
  "allowCustomerMismatch": zod.boolean().default(updateInvoiceBodyAllowCustomerMismatchDefault).describe('Explicit administrator override when source jobs belong to another customer'),
  "customerName": zod.string().nullish(),
  "customerIc": zod.string().nullish(),
  "customerDic": zod.string().nullish(),
  "customerAddress": zod.string().nullish(),
  ...
  "iban": zod.string().nullish(),
  "bic": zod.string().nullish(),
  "currency": zod.string().min(updateInvoiceBodyCurrencyMin).max(updateInvoiceBodyCurrencyMax).nullish(),
  "variableSymbol": zod.string().nullish(),
  "constantSymbol": zod.string().nullish(),
  "specificSymbol": zod.string().nullish(),
  "vatModeDefault": zod.enum(['standard', 'reverse_charge', 'zero', 'non_vat']).optional(),
...
})).optional().describe('Explicit settlement of every operational source independent of commercial line layout')
})
...
export const CancelInvoiceBody = zod.object({
  "reasonCode": zod.enum(['customer_complaint', 'incorrect_job', 'billing_error', 'duplicate_invoice', 'order_cancelled']).describe('Registered mandatory reason for the cancellation; arbitrary free text is not persisted'),
...
export const SetCostDocumentStatusBody = zod.object({
  ...
  "reason": zod.string().min(setCostDocumentStatusBodyReasonMin).max(setCostDocumentStatusBodyReasonMax).optional().describe('Povinný důvod při řízeném vrácení schváleného dokladu ke kontrole.')
})
...
export const DisposeCostDocumentBody = zod.union([zod.object({
  ...
  "reason": zod.string().min(disposeCostDocumentBodyTwoReasonMin).max(disposeCostDocumentBodyTwoReasonMax).describe('Bounded readable reason stored only in restricted immutable accounting evidence.')
})])
...
export const DisposeCostDocumentResponse = zod.object({
  ...
  "declaredDocType": zod.union([zod.literal('receipt'),zod.literal('delivery_note'),zod.literal('invoice'),zod.literal('credit_note'),zod.literal(null)]).nullish(),
  "detectedDocType": zod.union([zod.literal('receipt'),zod.literal('delivery_note'),zod.literal('invoice'),zod.literal('credit_note'),zod.literal(null)]).nullish(),
  "detectedDocTypeConfidence": zod.number().min(disposeCostDocumentResponseDocumentDetectedDocTypeConfidenceMin).max(disposeCostDocumentResponseDocumentDetectedDocTypeConfidenceMax).nullish(),
  "docTypeSource": zod.enum(['unknown', 'user', 'ai', 'conflict', 'admin']).optional(),
  "docTypeConfirmedAt": zod.string().nullish(),
  "source": zod.enum(['manual', 'job_attachment', 'isdoc', 'email']),
  "objectPath": zod.string().nullish(),
