...
const ACTION = "create-exact-0104-backup";
const LATEST_0104 = "0104_thin_sheva_callister";
const MAX_PAYLOAD_BYTES = 256 * 1024 * 1024;

export class StagingExact0104BackupRunnerError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "StagingExact0104BackupRunnerError";
    this.code = code;
  }
}

function fail(code, message) {
  throw new StagingExact0104BackupRunnerError(code, message);
}
...
function assertOnlyPostgresRunning(
  ...
  if (
    ...
    fail(
      "EXACT_0104_BACKUP_POSTGRES_CONTAINER_INVALID",
  ...
  try {
    ...
    fail(
      "EXACT_0104_BACKUP_POSTGRES_CONTAINER_MISMATCH",
...
function canonicalTimestamp(value, field) {
  if (typeof value !== "string" || !value.endsWith("Z")) {
    fail("EXACT_0104_BACKUP_TIME_INVALID", `${field} must be canonical UTC.`);
  }
...
function exactKeys(value, keys, field) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    fail("EXACT_0104_BACKUP_SCHEMA_INVALID", `${field} must be an object.`);
  }
  const actual = Object.keys(value).sort();
  const expected = [...keys].sort();
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail(
      "EXACT_0104_BACKUP_SCHEMA_INVALID",
...
function positiveInteger(value, field) {
  if (!Number.isSafeInteger(value) || value < 1) {
    fail("EXACT_0104_BACKUP_SCHEMA_INVALID", `${field} must be positive.`);
  }
  return value;
}
...
function parseTrustedInspectDeployment(
  ...
  if (
    ...
    fail(
      "EXACT_0104_BACKUP_INSPECT_INVALID",
...
function validateResolvedComposeTarget(
  ...
  try {
    ...
    fail(
      "EXACT_0104_BACKUP_COMPOSE_INVALID",
  ...
  try {
    ...
    fail(
      ...
      "Resolved Compose target does not match the approved inspect deployment.",
    );
  }
}

function parseTrustedBaseline(bytes, checksumText, expectedSha256) {
  if (
    !Buffer.isBuffer(bytes) ||
    bytes.length === 0 ||
    !SHA256.test(expectedSha256)
    ...
    fail(
      "EXACT_0104_BACKUP_BASELINE_INVALID",
      "The approved baseline execution bytes and SHA-256 are required.",
    );
  }
  const actualSha256 = crypto.createHash("sha256").update(bytes).digest("hex");
  if (
    actualSha256 !== expectedSha256 ||
    checksumText !== `${expectedSha256}  staging-baseline-0104-execution.json\n`
  ) {
    fail(
      "EXACT_0104_BACKUP_BASELINE_HASH_MISMATCH",
      "Baseline execution bytes do not match the separately approved checksum.",
    );
  }
  let value;
  try {
    ...
    fail(
      "EXACT_0104_BACKUP_BASELINE_INVALID",
      "Baseline execution must be strict JSON.",
    );
  }
  if (!bytes.equals(Buffer.from(canonicalJson(value), "utf8"))) {
    fail(
      "EXACT_0104_BACKUP_BASELINE_INVALID",
      "Baseline execution must use canonical JSON bytes.",
    );
  }
  exactKeys(
    value,
  ...
  if (
    ...
    !["migrate", "verified-noop"].includes(value.operation) ||
    value.requiresFreshExact0104BackupAndRestore !== true ||
    value.authorizes0105 !== false ||
    value.postcheck.phase !== "post" ||
    value.postcheck.operation !== "ready" ||
    value.postcheck.decision !== "READY_0104" ||
    !SHA40.test(String(value.postcheck.candidateSourceSha)) ||
    value.postcheck.appliedMigrations !== 104 ||
    value.postcheck.predecessorMigrations !== 104 ||
    value.postcheck.latestAppliedTag !== LATEST_0104 ||
    value.postcheck.missingToPredecessor !== 0 ||
    value.postcheck.authorizes0105 !== false ||
    positiveInteger(value.postcheck.backupEvidenceId, "baseline backup id") <
      1 ||
    ...
    fail(
      "EXACT_0104_BACKUP_BASELINE_INVALID",
      "Baseline execution does not prove the approved isolated exact-0104 state.",
    );
  }
  return Object.freeze({
    sha256: actualSha256,
    completedAt,
    sourceSha: value.postcheck.candidateSourceSha,
    backupId: value.postcheck.backupEvidenceId,
  });
}

function parseMarker(stdout) {
  const marker = "[staging-exact-0104-backup] PASS ";
  const lines = stdout.split(/\r?\n/).filter((line) => line.startsWith(marker));
  if (lines.length !== 1) {
    fail(
      "EXACT_0104_BACKUP_MARKER_INVALID",
...
function validateMarker(value, baseline, expectedSourceSha) {
  exactKeys(
    value,
    [
      "decision",
  ...
  if (
    ...
    value.buildSha !== baseline.sourceSha ||
    ...
    value.previousBackupId !== baseline.backupId ||
    positiveInteger(value.backupId, "new backup id") <= baseline.backupId ||
    ...
    positiveInteger(value.sizeBytes, "backup size") < 1 ||
    ...
    fail(
      "EXACT_0104_BACKUP_EVIDENCE_INVALID",
      "The one-shot evidence is not bound to the approved baseline and exact-0104 state.",
    );
  }
  return Object.freeze(value);
}

export function runStagingExact0104Backup({
  ...
  if (!SHA40.test(expectedSourceSha ?? "")) {
    fail(
      "EXACT_0104_BACKUP_SOURCE_INVALID",
  ...
  const baseline = parseTrustedBaseline(
    baselineExecutionBytes,
    baselineExecutionChecksumText,
    expectedBaselineExecutionSha256,
  );
  if (baseline.sourceSha !== expectedSourceSha) {
    fail(
      "EXACT_0104_BACKUP_SOURCE_MISMATCH",
      "The baseline execution and candidate source SHA differ.",
    );
  }
  ...
  if (inspectDeployment.inputs.backupEvidenceId !== baseline.backupId) {
    fail(
      "EXACT_0104_BACKUP_INSPECT_BACKUP_MISMATCH",
      "The inspect deployment and baseline execution must bind the same previous backup id.",
    );
  }
  const composeArgs = [
    "compose",
  ...
  try {
    stdout = checked(
      ...
      [
        ...
        "--no-deps",
        "-e",
        `STAGING_EXACT_0104_BACKUP_ACTION=${ACTION}`,
        "-e",
        `STAGING_EXACT_0104_BACKUP_CONFIRMATION=${CONFIRMATION}`,
        "exact-0104-backup",
        "node",
        "dist/external-schema-exact-0104-backup.mjs",
      ],
      "exact-0104 backup one-shot",
    );
  } finally {
  ...
  const gate = validateMarker(parseMarker(stdout), baseline, expectedSourceSha);
  return Object.freeze({
    schemaVersion: 1,
    kind: "site-logbook-staging-exact-0104-backup-execution",
    decision: "PASS",
    productionTargetsTouched: false,
    startedAt,
    completedAt: now().toISOString(),
    sourceSha: expectedSourceSha,
    baselineExecutionSha256: `sha256:${baseline.sha256}`,
...
export function prepareStagingExact0104BackupOutput(directory) {
  const absolute = path.resolve(directory);
  fs.mkdirSync(absolute, { recursive: true });
  const stat = fs.lstatSync(absolute);
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    fail(
      "EXACT_0104_BACKUP_OUTPUT_INVALID",
...
export function writeStagingExact0104BackupEvidence(directory, evidence) {
  const absolute = prepareStagingExact0104BackupOutput(directory);
  const name = "staging-exact-0104-backup-execution.json";
...
function requiredArgument(name) {
  ...
  if (!value) fail("EXACT_0104_BACKUP_INPUT_MISSING", `${name} is required.`);
  return value;
}
...
function regularFile(value, label) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    fail("EXACT_0104_BACKUP_INPUT_INVALID", `${label} must be a regular file.`);
  }
  return absolute;
}

function main() {
  const output = prepareStagingExact0104BackupOutput(
    requiredArgument("--output-dir"),
  );
  const baseline = regularFile(
    requiredArgument("--baseline-execution"),
    "baseline execution",
  );
  const checksum = regularFile(
  ...
  const evidence = runStagingExact0104Backup({
    ...
    baselineExecutionBytes: fs.readFileSync(baseline),
    baselineExecutionChecksumText: fs.readFileSync(checksum, "utf8"),
  ...
  const files = writeStagingExact0104BackupEvidence(output, evidence);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  try {
    ...
    const failure =
      error instanceof StagingExact0104BackupRunnerError
    ...
    process.stderr.write(
      `[staging-exact-0104-backup-runner] FAIL ${JSON.stringify(failure)}\n`,
    );
    process.exitCode = 1;
  }
}
