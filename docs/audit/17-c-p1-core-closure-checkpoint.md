...
- lokální R11-B řadí item locky a odstraňuje doložený A→B/B→A cross-source deadlock; R11-D po lock waitu znovu načte source ledger a nový nezamknutý target odmítne konfliktem; R11-F přidává sdílený batch item-lock plán a převádí cost-document stock lines. Ostatní vyšší transakce s několika oddělenými reconcile fázemi stále vyžadují převod na jeden společný plán před prvním reconcile;
