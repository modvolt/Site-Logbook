...
- **Lokální R11-H 2026-08-11:** ruční assign i automatický unambiguous warehouse material backfill používají společný movement batch. Automatický backfill už neprovádí dvě oddělené raw FK aktualizace bez ledgeru: běží v jedné transakci, drží stabilní unique-name mapu, má 5s lock a 30s statement timeout, zapisuje job/activity FK i odpovídající movements atomicky a při nekrytém výdeji vše rollbackne. Opakování je idempotentní a ambiguous names zůstávají nedotčené. Disposable warehouse suite po doplnění rollback testu: 19/19. Checkpoint `17-at-r11-warehouse-backfill-atomicity.md`.
