...
- **Lokální R11-G 2026-08-11:** `syncJobMaterialsForDocument()` sestaví jediný batch pro zaniklé i přežívající job/activity material zdroje, provede společný lock plán před prvním movementem a teprve po úspěchu smaže zaniklé source rows. Cílené PostgreSQL sady warehouse 17/17, material integrity 3/3 a document price propagation 19/19 prošly. Dvě historické fixture, které očekávaly výdej z nulového skladu, dostaly skutečný ledgerový počáteční příjem; nonnegative invariant se neobchází změnou cache. Vnější flow, které po sync batchi spouští další document batch, zatím stále potřebují společný outer plan. Checkpoint `17-as-r11-sync-material-batch.md`.
- **Lokální R11-H/I 2026-08-11:** následné review odmítlo původní bulk write cestu R11-H kvůli PostgreSQL `lower()` vs NFKC rozdílu, neuzavřenému bounded runtime a rizikovému table-lock pořadí. R11-I proto oba legacy bulk POSTy i helper fail-closed vypíná 409 a admin UI ponechává pouze read-only report. Source batch současně před discovery bere collision-free try advisory lock pro exact `(sourceType, sourceId)`, takže dvě první reconcile prázdného source do různých itemů nemohou obě commitnout. `cancel-last` vybírá pohyb až pod item lockem, používá authoritative nonnegative kontrolu a vrací exact vytvořený reversal. Disposable DB: warehouse FK 11/11 a ledger 22/22; statický containment 4/4. Outer multi-batch flows, restartovatelný NFKC maintenance plane, controlled override, row versions, live billing claim a oddělený migration plane zůstávají otevřené. Checkpoint `17-au-r11-source-cancel-backfill-containment.md`; `17-at` je historicky superseded.
