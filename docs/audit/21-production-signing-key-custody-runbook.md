...
Status: the Windows current-user custody helper, source-pinned trust-root
contracts and hermetic ceremony test are implemented. No production key has
been generated by this repository change. The two source maps deliberately
remain empty, so publisher verification, host attestation, backup-receipt
verification and production activation remain default-deny.

This is a preparation and operator ceremony, not deployment approval. It does
not change Coolify, GitHub, Hetzner, PostgreSQL, an object, a migration or a
running container.
