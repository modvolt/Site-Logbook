...
- Základ `8787d7f` už obsahuje opravu návaznosti přijaté faktury, dodacího listu, zakázky a materiálu i zpřesněné automatické párování. Faktura bez dodacího listu může projít po auditovaném rozhodnutí `not_required` s povinným důvodem; schválení následně synchronizuje potvrzené materiálové řádky na cílovou zakázku. Nejednoznačné nebo konfliktní automatické vazby se samy nepotvrdí.
...
- Aktuální lokální gate nad `8787d7f` je zelený: Node 329 pass + 1 PG16 skip, activation/DB 30 pass + 3 PG16 skip, frontend 193, live-events 15 a API 1069 pass + 1 skip. Cílený DB concurrency běh na novém disposable PostgreSQL 18 navíc prošel 11/11 scénářů a čtyři izolované DB sady vazeb/párování prošly 66/66 testů. Quality workflow nyní spouští nový 0108 lifecycle v samostatném disposable PostgreSQL 16 kontejneru; první skutečný PG16 důkaz musí ještě dodat PR CI.
...
- Převzaté opravy vazeb přijatá faktura/dodací list/zakázka/materiál a automatického párování: 4 izolované DB soubory na disposable PostgreSQL 18, 66/66 testů.
