  ...
  return (
    <section className="mt-5 border-y bg-card">
      ...
      <div className="divide-y">{phase.items.map((item) => {
        ...
        const canEdit = responsePermission && (item.kind !== "measurement" || can("switchboards.measurements.create")) && (item.kind !== "photo" || can("switchboards.photos.create"));
      ...
      <div className="p-4 border-t space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          {can("switchboards.phases.complete") && <Button disabled={!isOnline || completePhase.isPending || phase.summary.completed < phase.summary.total || phase.summary.status === "completed" || (phase.summary.criticalDefects > 0 && (!can("switchboards.protocol.override") || overrideReason.trim().length < 10))} onClick={() => completePhase.mutate({ phaseKey: phase.key, reason: overrideReason.trim() || undefined })}>{completePhase.isPending ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <CheckCircle2 className="h-4 w-4 mr-1" />}Dokončit fázi</Button>}
        </div>
        {can("switchboards.protocol.override") && phase.summary.status !== "completed" && <div className="space-y-1.5"><Label htmlFor={`phase-override-${phase.key}`} className="text-xs">Odůvodnění administrátorské výjimky</Label><Textarea id={`phase-override-${phase.key}`} rows={2} value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} placeholder="Vyplňte pouze při vědomém obejití kritické blokace (min. 10 znaků)." /></div>}
      </div>
    </section>
  );
}
