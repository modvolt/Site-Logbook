...
const EMPTY: FormState = { name: "", code: "", salePrice: "" };
...
export default function Sklad() {
  ...
  const formFields = (f: FormState, set: (f: FormState) => void) => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      <Input value={f.name} onChange={(e) => set({ ...f, name: e.target.value })} placeholder="Název *" className="h-12 bg-background" />
              ...
              <Card key={item.id} className="hover:bg-muted/50 transition-colors">
                <CardContent className="p-4 flex justify-between items-center gap-3">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <p className="font-semibold">{fmtKc(item.salePrice)}</p>
                    {can("write") && (
