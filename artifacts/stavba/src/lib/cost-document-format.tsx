...
const DELIVERY_NOTE_WORKFLOW_LABELS: Record<string, string> = {
  needs_decision: "Určete dodací list",
  waiting_for_delivery_note: "Čeká na dodací list",
  ready: "Dodací listy připraveny",
  ready_without_delivery_note: "Bez dodacího listu",
};

const DELIVERY_NOTE_WORKFLOW_CLASSES: Record<string, string> = {
  needs_decision:
...
export function DeliveryNoteWorkflowBadge({ state }: { state: string }) {
  if (state === "not_applicable") return null;
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
        DELIVERY_NOTE_WORKFLOW_CLASSES[state] ??
        "bg-muted text-muted-foreground"
      }`}
    >
      {DELIVERY_NOTE_WORKFLOW_LABELS[state] ?? state}
    </span>
  );
}
