...
describe("immutable job and quote evidence at the database boundary", () => {
  it("allows the one job signing transition and then rejects tamper/delete", async () => {
    ...
    const [version] = await db.insert(jobDocumentVersionsTable).values({
      jobId: job!.id,
      version: 1,
    ...
    await expect(db.update(jobDocumentVersionsTable).set({
      status: "signed",
      ...
      pdfSha256: HASH_B,
      signedAt,
    }).where(eq(jobDocumentVersionsTable.id, version!.id))).resolves.toBeDefined();

    await expect(db.update(jobDocumentVersionsTable)
    ...
    await expect(db.delete(jobDocumentVersionsTable)
      .where(eq(jobDocumentVersionsTable.id, version!.id)))
      .rejects.toBeDefined();
    await expect(db.select({ id: jobDocumentVersionsTable.id })
      .from(jobDocumentVersionsTable)
      .where(eq(jobDocumentVersionsTable.id, version!.id)))
      .resolves.toHaveLength(1);

    const [event] = await db.insert(jobSignatureEventsTable).values({
      jobId: job!.id,
    ...
    await expect(db.update(jobSignatureEventsTable)
  ...
  it("rejects quote version/event mutation and invalid bound tokens", async () => {
    const [quote] = await db.insert(quotesTable).values({ title: TAG, status: "sent" }).returning();
    ...
    await expect(db.update(quoteVersionsTable)
    ...
    await expect(db.delete(quoteVersionsTable)
      .where(eq(quoteVersionsTable.id, version!.id)))
      .rejects.toBeDefined();
    await expect(db.select({ id: quoteVersionsTable.id })
      .from(quoteVersionsTable)
      .where(eq(quoteVersionsTable.id, version!.id)))
      .resolves.toHaveLength(1);

    const [event] = await db.insert(quoteDecisionEventsTable).values({
      quoteId: quote!.id,
    ...
    await expect(db.delete(quoteDecisionEventsTable)
      .where(eq(quoteDecisionEventsTable.id, event!.id)))
      .rejects.toBeDefined();
    await expect(db.select({ id: quoteDecisionEventsTable.id })
      .from(quoteDecisionEventsTable)
      .where(eq(quoteDecisionEventsTable.id, event!.id)))
      .resolves.toHaveLength(1);

    await expect(db.insert(publicAccessTokensTable).values({
    ...
    await expect(db.select({ id: publicAccessTokensTable.id })
