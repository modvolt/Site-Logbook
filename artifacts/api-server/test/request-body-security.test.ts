...
describe("API body limit policy", () => {
  it("uses a small default/public cap and only named authenticated large routes", () => {
    expect(
      jsonBodyLimitForRequest({ method: "POST", originalUrl: "/api/jobs" }),
    ).toBe(API_BODY_LIMITS.authenticatedJsonBytes);
    expect(
      jsonBodyLimitForRequest({
        method: "POST",
        originalUrl: "/api/sign/token",
      }),
    ...
    for (const originalUrl of [
      ...
      expect(jsonBodyLimitForRequest({ method: "POST", originalUrl })).toBe(
        API_BODY_LIMITS.publicJsonBytes,
      );
    }
    expect(
      jsonBodyLimitForRequest({
        method: "POST",
        originalUrl: "/api/jobs/42/job-sheet",
      }),
    ).toBe(API_BODY_LIMITS.largeAuthenticatedJsonBytes);
    expect(
      jsonBodyLimitForRequest({
        method: "POST",
        originalUrl: "/api/billing/bank-statements/parse?x=1",
      }),
    ).toBe(API_BODY_LIMITS.largeAuthenticatedJsonBytes);
    expect(
      jsonBodyLimitForRequest({
...
describe("middleware and proxy ordering contract", () => {
  it("authenticates and authorizes before parsing, then fingerprints the parsed body", () => {
    ...
    expect(
      source.indexOf('app.use("/api", limitPublicBearerRequests)'),
    ).toBeLessThan(source.indexOf('app.use("/api", parseApiRequestBody)'));
    expect(
      source.indexOf('app.use("/api", requireOnlineIdempotencyStepUp)'),
    ).toBeLessThan(source.indexOf('app.use("/api", parseApiRequestBody)'));
    expect(
      source.indexOf('app.use("/api", enforceDurableIdempotency)'),
    ).toBeGreaterThan(source.indexOf('app.use("/api", parseApiRequestBody)'));
