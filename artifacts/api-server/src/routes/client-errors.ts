 ...
 */
export function startClientErrorPurgeScheduler(): SchedulerStopHandle {
  ...
  const tick = (): void => {
    ...
    purgeOldClientErrors(days)
      .then((deleted) => {
        if (deleted > 0) {
          logger.info(
            { deleted, retentionDays: days },
            "Purged old client error records",
          );
        }
      })
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    req.log.warn(
      { clientError: { message, path, userRole } },
      "Client-side crash reported",
    );
    res.status(204).end();
  },
);
