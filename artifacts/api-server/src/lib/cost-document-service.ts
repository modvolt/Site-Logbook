...
import { CONFIDENCE_REVIEW_THRESHOLD } from "./openai-extraction";
...
async function persistDocumentRelationship(
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.insert(auditLogTable).values({
      ...
      entityType: "billing_documents",
      entityId: invoice.id,
      summary: `Doklad automaticky propojen s dodacim listem #${deliveryNote.id} (shoda ${Math.round(
  ...
  await db.transaction(async (tx) => {
    ...
    const lowConfidenceWarning =
      suggestion.confidence < CONFIDENCE_REVIEW_THRESHOLD
        ? `ALARM: Spolehlivost automatickeho vytezeni je pouze ${Math.round(
            suggestion.confidence * 100,
          )} %. Doklad vyzaduje rucni kontrolu.`
        : null;
