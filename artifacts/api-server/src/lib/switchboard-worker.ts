...
async function processOne(id: number) {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      const reviewCode = result.status === "label_not_found" ? "label_not_found" : ("ambiguousPages" in result && result.ambiguousPages.length ? "multiple_label_candidates" : ("missingRequired" in result && result.missingRequired.length ? "missing_required_fields" : null));
      const reviewMessage = reviewCode === "label_not_found" ? "Typový štítek nebyl nalezen." : reviewCode === "multiple_label_candidates" ? "Bylo nalezeno více rovnocenných kandidátů typového štítku." : reviewCode === "missing_required_fields" ? "Některá povinná pole chybí nebo nemají dostatečnou jistotu." : null;
      await tx.update(switchboardDocumentsTable).set({ processingStatus: finalStatus, processingErrorCode: reviewCode, processingErrorMessage: reviewMessage }).where(eq(switchboardDocumentsTable.id, document.id));
