...
import {
  ...
  switchboardExtractedFieldsTable,
  switchboardFieldRegistryTable,
...
import {
  parseSwitchboardLabel,
  SWITCHBOARD_PARSER_VERSION,
  type FieldDefinition,
} from "./switchboard-parser";
import {
  extractPdfOcrElements,
  extractPdfTextElements,
  shouldUseOcrFallback,
} from "./switchboard-pdf";
...
async function processOne(id: number) {
  const [claimed] = await db
    .update(switchboardProcessingJobsTable)
    .set({
      status: "running",
      attemptCount: sql`${switchboardProcessingJobsTable.attemptCount} + 1`,
      startedAt: new Date(),
      lockedAt: new Date(),
      lockedBy: process.env.HOSTNAME ?? "api",
    })
    .where(
      and(
        eq(switchboardProcessingJobsTable.id, id),
        eq(switchboardProcessingJobsTable.status, "queued"),
      ),
    )
    .returning();
  if (!claimed) return;
  ...
  try {
    const [document] = await db
      .select()
      .from(switchboardDocumentsTable)
      .where(eq(switchboardDocumentsTable.id, claimed.documentId));
    if (!document)
      throw Object.assign(new Error("Dokument neexistuje."), {
        code: "document_missing",
      });
    switchboardId = document.switchboardId;
    await db
      .update(switchboardDocumentsTable)
      .set({ processingStatus: "analyzing_pdf" })
      .where(eq(switchboardDocumentsTable.id, document.id));
    const buffer = await storage.getPrivateObjectBuffer(document.storagePath);
    let extracted = await extractPdfTextElements(buffer);
    let registryRows = await db
      .select()
      .from(switchboardFieldRegistryTable)
      .where(eq(switchboardFieldRegistryTable.isActive, true));
    const registry: FieldDefinition[] = registryRows.map((field) => ({
      ...field,
      minimumConfidence: Number(field.minimumConfidence),
    }));
    let result = parseSwitchboardLabel(extracted.elements, registry);
    if (shouldUseOcrFallback(result.status)) {
      await db
        .update(switchboardDocumentsTable)
        .set({ processingStatus: "ocr" })
        .where(eq(switchboardDocumentsTable.id, document.id));
      ...
      result.status === "label_not_found"
        ...
        : "ambiguousPages" in result && result.ambiguousPages.length
          ...
          : "ambiguousFields" in result && result.ambiguousFields.length
            ...
            : "missingRequired" in result && result.missingRequired.length
              ? "missing_required_fields"
              : null;
    const reviewMessage =
      reviewCode === "label_not_found"
    ...
    const auditPayload = {
      ...
      extractionMethod: extracted.elements.some(
        (element) => element.method === "ocr",
      )
        ? "ocr"
        : "text_layer",
      ...
      ambiguousFields:
        "ambiguousFields" in result ? result.ambiguousFields : [],
    ...
    await db.transaction(async (tx) => {
      await tx
        .delete(switchboardExtractedFieldsTable)
        .where(eq(switchboardExtractedFieldsTable.documentId, document.id));
      if (result.fields.length)
        await tx.insert(switchboardExtractedFieldsTable).values(
      ...
      await tx
        ...
        .set({
          ...
          processingErrorCode: reviewCode,
          processingErrorMessage: reviewMessage,
        })
        .where(eq(switchboardDocumentsTable.id, document.id));
      await tx
        ...
        .set({
          ...(complete
            ? switchboardPatchFromExtractedFields(result.fields)
            : {}),
          processingStatus: intermediateStatus,
      ...
      if (!complete) {
        await tx
          ...
          .set({
            ...
            errorCode: reviewCode,
            errorMessage: reviewMessage,
          })
          .where(eq(switchboardProcessingJobsTable.id, claimed.id));
        await tx.insert(switchboardEventsTable).values({
          switchboardId: document.switchboardId,
          eventType: "document_processed",
          entityType: "switchboard_document",
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.insert(switchboardEventsTable).values({
        ...
        payload: {
          ...auditPayload,
          labelVersion: labelResult.label.version,
          labelCreated: labelResult.created,
          qrActivated: labelResult.qrActivated,
        },
        actorName: "System",
      });
    });
    ...
    const message =
      error instanceof Error ? error.message.slice(0, 1000) : "Neznámá chyba.";
    const code = (error as { code?: string }).code ?? "processing_failed";
    const requiresReview = [
      "qr_inactive",
      "qr_expired",
      "label_fields_missing",
    ].includes(code);
    const exhausted =
      requiresReview || claimed.attemptCount >= claimed.maxAttempts;
    const documentStatus = requiresReview
      ? "needs_review"
      : exhausted
        ? "failed"
        : "queued";
    ...
    if (switchboardId) {
      await db
        .update(switchboardsTable)
        .set({ processingStatus: documentStatus, updatedAt: new Date() })
        .where(eq(switchboardsTable.id, switchboardId));
      if (requiresReview)
        await db.insert(switchboardEventsTable).values({
          switchboardId,
          eventType: "label_generation_blocked",
          entityType: "switchboard_document",
    ...
    logger.error(
      { err: error, switchboardProcessingJobId: claimed.id },
      "Switchboard document processing failed",
    );
  }
}

export async function drainSwitchboardQueue(signal?: AbortSignal) {
  if (signal?.aborted || draining) return;
  draining = true;
  try {
    await db
      .update(switchboardProcessingJobsTable)
      .set({
        ...
        errorMessage: "Úloha obnovena po restartu serveru.",
      })
      .where(
        and(
          eq(switchboardProcessingJobsTable.status, "running"),
          lt(
            switchboardProcessingJobsTable.lockedAt,
            new Date(Date.now() - STALE_MS),
          ),
        ),
      );
    if (signal?.aborted) return;
    const jobs = await db
      .select({ id: switchboardProcessingJobsTable.id })
      .from(switchboardProcessingJobsTable)
      .where(
        and(
          eq(switchboardProcessingJobsTable.status, "queued"),
          lte(switchboardProcessingJobsTable.availableAt, new Date()),
        ),
      )
      .orderBy(asc(switchboardProcessingJobsTable.availableAt))
      .limit(2);
...
export function startSwitchboardWorker(): SchedulerStopHandle {
  ...
  const tick = (): void => {
    if (stopped) return;
    void drainSwitchboardQueue(abortController.signal);
  };
