...
export const ExtractionResultSchema = z.object({
  docType: z
    ...
    .optional()
    .transform((v) => v ?? null),
  docTypeConfidence: nullableNumber.transform((v) =>
  ...
  // Multi-page completeness signals. A photographed page can contain valid line
  // items while still being only page 1/N; the backend uses these fields to
  // prevent approving/ignoring an incomplete page as a standalone document.
...
export function normalizeResult(
  ...
  if (
    ...
    result.pageCount > 1 &&
    result.finalTotalPresent === false
  ) {
    warnings.push(
      `NEUPLNY_VICESTRANKOVY_DOKLAD: Viditelná je pouze strana ${result.pageNumber ?? "?"}/${result.pageCount} a chybí finální součet dokladu. Stránku je nutné sloučit s dalšími stranami před schválením nebo ignorováním.`,
    );
  }
...
export const DEFAULT_SYSTEM_PROMPT = `Jsi přesný asistent pro vytěžování českých účetních dokladů (faktury, účtenky, dodací listy, dobropisy) pro stavební firmu.
...
Z přiloženého dokladu vyčti údaje a vrať VÝHRADNĚ jeden JSON objekt (žádný další text, žádné markdown bloky) s tímto tvarem:
{
  ...
  "pageNumber": number | null,         // číslo viditelné strany, pokud je uvedeno (např. 1 u "Strana 1/2")
  "pageCount": number | null,          // celkový počet stran, pokud je uveden
  "finalTotalPresent": boolean | null, // true jen když je vidět finální součet celého dokladu
...
Pravidla:
- "docType" VŽDY aktivně vyhodnoť podle skutečného obsahu dokladu — nikdy paušálně nepředpokládej fakturu. Rozlišovací znaky:
  - "invoice" (faktura): má variabilní symbol, datum splatnosti a částku k úhradě (s DPH); bývá nadepsaná „Faktura", „Faktura – daňový doklad" nebo „Daňový doklad".
  - "delivery_note" (dodací list): obsahuje dodané položky a množství, ale typicky NEMÁ variabilní symbol, datum splatnosti ani částku k úhradě; bývá nadepsaná „Dodací list" (případně „Delivery note").
  - "receipt" (účtenka): pokladní/zjednodušený daňový doklad o úhradě (hotově nebo kartou), obvykle menšího rozsahu; bývá nadepsaná „Účtenka", „Pokladní doklad" nebo „Paragon".
  - "credit_note" (dobropis): opravný daňový doklad s vrácením / zápornými částkami; bývá nadepsaný „Dobropis" nebo „Opravný daňový doklad".
  - Když si typem nejsi jistý, zvol nejpravděpodobnější variantu, sniž "confidence" a důvod nejistoty uveď ve "warnings".
- "docTypeConfidence" je jistota pouze klasifikace typu dokladu v rozsahu 0..1.
...
- "finalTotalPresent" nastav na true pouze tehdy, když je na zpracovaném vstupu vidět FINÁLNÍ součet celého dokladu (např. "Celkem k úhradě", "Doklad celkem", rekapitulace DPH / základ + DPH + celkem). Nenastavuj true kvůli řádkovému sloupci "Celkem bez DPH" nebo mezisoučtu jednotlivých položek.
- Pokud je vidět "Strana X/Y", vyplň "pageNumber" a "pageCount". U vícestránkového dokladu bez finálního součtu sniž "confidence" a uveď ve "warnings", že doklad není kompletní.
